...
parser = argparse.ArgumentParser(description= 'Train models for NABirds data')
...
if params.small:
    DATA_TAG = 'nabirdS' #Use small training dataset
...
if AUG:
    ...
    #aug for inceptionv3:
    '''aug = transforms.Compose([transforms.RandomAffine(degrees=8.441441791115345, 
                        translate=(11.824603752142139/128,11.877376943697527/128), 
                        scale=(0.902614449549031, 1.1078927447922258), shear=9.77268176670485),transforms.ToTensor()])'''
    
    #aug for resnet50:
    '''aug = transforms.Compose([transforms.RandomAffine(degrees=6.915425743887091, 
                        translate=(9.361539194580473/128,9.090457757001786/128), 
                        scale=(0.9476760158747418,1.0552129454040906), shear=7.612769875579133),transforms.ToTensor()])'''
...
trainloader2 = DataLoader(MapDataset(train_set, transforms.ToTensor()), batch_size=200, shuffle=True) #To analyze training
...
testloader2 = DataLoader(test_set_tf, batch_size=200, shuffle=True) #To analyze training
...
class Net(nn.Module):
    ...
    def forward(self):
        ...
        arc_cos_W = torch.acos(torch.clip(cos_sim_W,-1,1)*(1-1e-6)) 
...
if params.mds_W:
    ...
    if params.distance=='arccos':
        ...
        raise
    #print('max dist:', np.max(true_dist2))
...
model_PATH_ = 'models_'+DATA_TAG+'/' #'models_nabird/'
if use_val:
...
for epoch in range(tot_epoch):  # loop over the dataset multiple times    
    ...
    plt.savefig('loss_plot_nabird',dpi=200)
    plt.close()
    print()
    sys.stdout.flush()
